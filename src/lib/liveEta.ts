...
export function etaMinutesFromGps(params: {
  ...
  /** Optional geocoded job coords; when missing, fall back to knownMiles or stored etaMinutes */
  jobLat?: number | null;
  jobLng?: number | null;
  /** Precomputed miles (e.g. booking.distance_miles) when job lat/lng unavailable */
  knownMiles?: number | null;
  fallbackEtaMinutes?: number | null;
}): { minutes: number | null; source: 'gps' | 'stored' | 'none'; miles: number | null } {
  const { techLat, techLng, jobLat, jobLng, knownMiles, fallbackEtaMinutes } = params;
  if (
    techLat != null &&
    techLng != null &&
    jobLat != null &&
  ...
  if (
    ...
    const minutes = Math.max(1, Math.round((knownMiles / MPH) * 60));
    return { minutes, source: 'gps', miles: Math.round(knownMiles * 10) / 10 };
  }
